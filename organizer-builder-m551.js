(function(){
'use strict';
var cfg=window.IWP_SITE_CONFIG||{},appUrl=String(cfg.appUrl||''),sessionKey='iwpOrganizerSessionV1',request=0,timer=0,eventId=new URLSearchParams(location.search).get('id')||'',pendingUpload=null,isDirty=false,isSaving=false;
...
function submitSave(){document.addEventListener('keydown',function(e){if((e.ctrlKey||e.metaKey)&&String(e.key).toLowerCase()==='s'){e.preventDefault();var button=byId('saveAdventureButton');if(button&&!button.disabled){window.IWPToast&&window.IWPToast('Saving adventure draft…','info','Save Draft');button.click()}}});
var session=readSession();if(!session){fail();return}var data=payload();if(!validateSaveData(data))return;var b=byId('saveAdventureButton');isSaving=true;b.disabled=true;b.textContent='Saving…';text('builderStatus','Saving draft');text('builderMessage','Writing the adventure to the live Community Connections database.');jsonp('organizer-save-adventure','iwpOrganizerSaveAdventureCallback',{session:session.token,eventId:eventId,data:JSON.stringify(data)},'builderSave')}
byId('builderForm').addEventListener('submit',function(e){e.preventDefault();if(pendingUpload)uploadImage(pendingUpload);else submitSave()});
