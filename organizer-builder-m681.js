...
window.iwpOrganizerSaveAdventureCallback=function(p){clearTimer();remove('builderSave');isSaving=false;var b=byId('saveAdventureButton'),pub=byId('publishAdventureButton');b.disabled=false;if(pub)pub.disabled=false;if(!p||!p.success){b.textContent=currentEventStatus==='published'?'Update Adventure':'Save Draft';if(pub)pub.textContent='Publish Adventure';text('builderStatus',saveMode==='published'?'Publish failed':'Save failed');text('builderMessage',p&&p.error||'Unable to save the adventure.');return}eventId=p.eventId||eventId;clearPublicAdventureCache(eventId);isDirty=false;currentEventStatus=String(p.event&&(p.event.Status||p.event.status)||saveMode||currentEventStatus).toLowerCase();text('builderStatus',currentEventStatus==='published'?'Adventure published':(p.created?'Draft created':'Draft updated'));text('builderMessage',currentEventStatus==='published'?'Adventure is live. Returning to Manage Adventures…':'Adventure saved. Returning to Manage Adventures…');window.setTimeout(function(){window.location.replace('/organizer-adventures.html?saved='+encodeURIComponent(eventId)+'&v=m6.21.1')},250)};
function payload(){var out={};['Title','EventType','ImageUrl','LocationName','Address','Description','WhatToExpect','WhatToBring','Provided','SpecialNotes','AdultCost','ChildCost','TicketPurchaseLink','MaxParticipants','OrganizerName','OrganizerEmail','OrganizerPhone'].forEach(function(k){out[k]=byId(k).value.trim()});out.StartDate=displayDateToApi(byId('StartDate').value);out.EndDate=displayDateToApi(byId('EndDate').value);out.StartTime=document.querySelector('[data-time-field="StartTime"]')._get();out.EndTime=document.querySelector('[data-time-field="EndTime"]')._get();['RegistrationRequired','ChildrenAllowed','WaitlistEnabled','FreeEvent','BuyOwnTicketsEnabled'].forEach(function(k){out[k]=byId(k).checked});out.PaidEvent=!out.FreeEvent;if(out.FreeEvent){out.AdultCost='';out.ChildCost='';out.BuyOwnTicketsEnabled=false;out.TicketPurchaseLink=''}else if(out.BuyOwnTicketsEnabled){out.AdultCost='';out.ChildCost=''}out.Status=saveMode==='published'?'Published':(currentEventStatus==='published'?'Published':'Draft');return out}
