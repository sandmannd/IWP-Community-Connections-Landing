...
export async function onRequestPost({request,env}){
  try{
    ...
    const event=await env.COMMUNITY_DB.prepare('SELECT event_id,status,title,organizer_email FROM events WHERE event_id=?').bind(eventId).first();
    if(!event) throw new Error('Adventure not found.');
    const now=new Date().toISOString();
    if(action==='publish'){
      const organizerEmail=String(event.organizer_email||'').trim();
      if(!organizerEmail) throw new Error('Organizer email is required before an adventure can be published.');
      if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(organizerEmail)) throw new Error('Enter a valid organizer email before publishing.');
      await env.COMMUNITY_DB.prepare("UPDATE events SET status='Published',updated_at=? WHERE event_id=?").bind(now,eventId).run();
      return json({success:true,authorized:true,user,eventId,message:'Adventure published.',source:'d1',migration:'M7.8.1'});
    }
    if(action==='cancel'){
      await env.COMMUNITY_DB.prepare("UPDATE events SET status='Cancelled',updated_at=? WHERE event_id=?").bind(now,eventId).run();
      return json({success:true,authorized:true,user,eventId,message:'Adventure cancelled. Registrations were preserved.',source:'d1',migration:'M7.8.1'});
    }
    if(action==='close'){
      await env.COMMUNITY_DB.prepare("UPDATE events SET status='Closed',updated_at=? WHERE event_id=?").bind(now,eventId).run();
      return json({success:true,authorized:true,user,eventId,message:'Adventure registration closed.',source:'d1',migration:'M7.8.1'});
    }
    if(action==='delete'){
      ...
      return json({success:true,authorized:true,user,eventId,message:'Adventure permanently deleted.',source:'d1',migration:'M7.8.1'});
    }
